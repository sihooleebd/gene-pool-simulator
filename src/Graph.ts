...
class Graph {
  ...
  lineTo(ctx: CanvasRenderingContext2D, p: PointT, lineWidth: number) {
  ...
  drawAxis(ctx: CanvasRenderingContext2D) {
    ...
    if (this.xFrom <= 0 && this.xTo >= 0) {
      ...
      this.lineTo(ctx, { x: this.xTo + xMargin, y: 0 }, 1);
    ...
    if (this.yFrom <= 0 && this.yTo >= 0) {
      ...
      this.lineTo(ctx, { x: 0, y: this.yTo + yMargin }, 1);
      ctx.lineTo(x - arrowSize, y + arrowSize);
